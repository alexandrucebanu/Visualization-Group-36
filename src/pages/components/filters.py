...
def layout(sourceDF,chosenPlayer):
    ...
    wage_min = int(min(wageSeries) // 10000 * 10000)
    wage_max = int(max(wageSeries) // 10000 * 10000)
    if max(wageSeries) % 10000 != 0:
        wage_max += 10000

    marks = {i: f'{i // 1000}k' for i in range(wage_min, wage_max + 1, 50000) if i % 50000 == 0}
    ...
    return html.Div([html.Div([
        ...
        html.Div(
            ...
            dcc.RangeSlider(min(wageSeries), max(wageSeries), value=[min(wageSeries), max(wageSeries)],
                tooltip={"placement": "bottom", "always_visible": True},
                id='wage_slider', step=20000, marks=marks)],
