...
def applyFilters(value, chosenPositions, filters):
    ...
    filtered_data = filterDataFrame[
        (filterDataFrame['gca'] >= x_min) & (filterDataFrame['gca'] <= x_max) &
        (filterDataFrame['passes_completed'] >= y_min) & (filterDataFrame['passes_completed'] <= y_max)
    ]
...
# -------------------------------------------------------------
@callback([Output(component_id='graph1_general', component_property='figure'), Output(component_id='graph2_general', component_property='figure')], Output('filters2', 'data'), 
            Input('filters', 'data'))  # Updates the general plots based on filter
def update_general_plots(filters):
    filterDataFrame = getFilteredDF(filters)
    #print(filterDataFrame['height_cm'])
    ...
    try:
        ...
        fig22 = px.scatter(filterDataFrame, x="passes_completed", y='gca', title='Relation between agility and physical properties', labels={'x': 'Height x Weight [cm * kg]', 'y': 'Agility in Movement'}, hover_data=['player'])
